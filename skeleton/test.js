...
test("addTodo Function logic work", function(t) {
  t.deepEqual(
    logic.addTodo([], { des: "wenak", done: false }),
    [{ id: 1, des: "wenak", done: false }],
    "Should add newTodo Object to the todos array test1"
  );
  t.deepEqual(
    logic.addTodo([{ id: 1, des: "wenak", done: false }], {
      des: "suha",
      done: false
    }),
    [{ id: 1, des: "wenak", done: false }, { des: "suha", done: false, id: 2 }],
    "Should add newTodo Object to the todos array test2"
  );
  t.deepEqual(
    logic.addTodo(
      [{ id: -2, des: "GO" }, { id: 1, des: "wenak", done: false }],
      { des: "suha", done: false }
    ),
    [
      { id: -2, des: "GO" },
      { id: 1, des: "wenak", done: false },
      { des: "suha", done: false, id: 3 }
    ],
    "Should add newTodo Object to the todos array test3"
  );
  ...
  t.equal(
    Array.isArray(logic.addTodo([], "hi")),
    true,
...
test("refacrtor markTodo function", function(t) {
  ...
  t.deepEqual(
    logic.markTodo([obj1], 0),
    [{ id: 0, desc: "rrr", done: true }],
    "return the same array with markTodo changed for the given id"
  );
  t.deepEqual(
    logic.markTodo([obj1], 2),
    [obj1],
    "return the same array if given invalid id"
  );
  t.end();
});
